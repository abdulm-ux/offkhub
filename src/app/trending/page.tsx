...
export default async function TrendingPage() {
  const { data: materials } = await supabase
    .from("materials")
    .select("id, title, type, download_count, course_id, department_id, courses(code, departments(slug, schools(slug))), departments(name, slug, schools(slug))")
    .eq("approved", true)
  ...
  return (
    <div>
      <h1 className="font-display text-2xl sm:text-3xl font-semibold text-paper mb-2">🔥 Trending</h1>
      <p className="text-paper/50 text-sm mb-8">Most downloaded materials across every school.</p>
