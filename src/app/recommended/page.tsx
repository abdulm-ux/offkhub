...
export default async function RecommendedPage() {
  const { data: materials } = await supabase
    .from("materials")
    .select("id, title, type, course_id, department_id, courses(code, departments(slug, schools(slug))), departments(name, slug, schools(slug))")
    .eq("approved", true)
    .eq("recommended", true)
    .limit(40);
  ...
  return (
    <div>
      <h1 className="font-display text-2xl sm:text-3xl font-semibold text-paper mb-2">⭐ Lecturer Recommended</h1>
      <p className="text-paper/50 text-sm mb-8">Handpicked by admins as especially useful.</p>
        ...
        {materials?.map((m: any, i) => {
          ...
          return (
            <Link key={m.id} href={href} className="flex items-center gap-4 bg-paper text-ink rounded-sm px-4 py-3 hover:bg-paper/90 transition-colors">
              ...
              <span className="font-mono text-xs text-tape shrink-0">⭐</span>
            </Link>
          );
        })}
